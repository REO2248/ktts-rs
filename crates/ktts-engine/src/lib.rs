...
#[derive(Debug)]
pub struct Engine {
    kma: ktts_kma::KmaContext,
    pron: ktts_pron::PronContext,
    prosody: ktts_prosody::ProsodyContext,
    synth: ktts_synth::SynthContext,
}

impl Engine {
    /// Loads all dictionaries once for repeated synthesis calls.
    ///
    /// # Errors
    ///
    /// Returns the first engine-stage loading error.
    pub fn load(files: DataMap, voice: &str) -> Result<Self, PipelineError> {
        ...
        let prosody = ktts_prosody::load_prosody_dicts_bytes(&files, voice).map_err(|e| {
            PipelineError::Engine("ktts-prosody", format!("load_prosody_dicts: {e}"))
        })?;
        let synth = ktts_synth::load_synth_db_bytes(files, voice)
    ...
    /// Synthesizes text with the loaded dictionaries.
    ///
    /// # Errors
    ///
    /// Returns an error if any engine stage fails.
    pub fn synthesize(&self, text: &str, params: &VoiceParams) -> Result<Vec<i16>, PipelineError> {
        let words = stage_kma(text, &self.kma)?;
        let pron = stage_pron(&words, &self.pron)?;
        let mut targets = stage_prosody(&pron, &words, &self.prosody)?;
        apply_sentence_end_boundaries(&pron, &words, &mut targets);
        if targets.is_empty() {
            return Ok(Vec::new());
        }
        stage_synth(&pron, &targets, &self.synth, params)
    }
}

fn stage_kma(text: &str, ctx: &ktts_kma::KmaContext) -> Result<Vec<WordAnal>, PipelineError> {
...
fn stage_pron(words: &[WordAnal], ctx: &ktts_pron::PronContext) -> Result<PronText, PipelineError> {
...
fn stage_prosody(
    pron: &PronText,
    words: &[WordAnal],
    ctx: &ktts_prosody::ProsodyContext,
...
fn stage_synth(
    ...
    ktts_synth::synthesize_with_params(ctx, &input, &tgts, *params)
...
mod tests {
    ...
    fn engine_load_reports_the_failing_stage() {
        ...
        assert!(
            matches!(err, PipelineError::Engine("ktts-kma", _)),
    ...
    fn test_files() -> DataMap {
        ...
        while let Some(dir) = stack.pop() {
            for entry in std::fs::read_dir(&dir).expect("read dictionary directory") {
                let path = entry.expect("dictionary entry").path();
                if path.is_dir() {
                    stack.push(path);
                } else {
                    let rel = path
                        .strip_prefix(&root)
                        .expect("path below dictionary root");
                    files.insert(
                        rel.to_string_lossy().replace('\\', "/"),
                        std::fs::read(&path).expect("read dictionary file"),
                    );
                }
            }
        }
