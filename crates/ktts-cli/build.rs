//! Embeds the kttsdb dictionary data into the binary when the `embed`
//! feature is enabled (`cargo build --features embed`).
//!
//! The data is packed into a simple length-prefixed blob written to
...
fn main() {
    ...
    eprintln!(
        "ktts(embed): packed {} files ({} bytes) from {}",
        files.len(),
        blob.len(),
        root.display()
    );
